...
if (!$notFound && $property !== null && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf($_POST['csrf_token'] ?? null)) {
        ...
        redirect('seller/property_edit.php?id=' . $propertyId);
        ...
        unset($_POST['listed_by_user_id'], $_POST['status']);
        ...
        if ($errors === []) {
            try {
                $pdo = db();
                $currentStatus = strtoupper((string) ($property['status'] ?? 'PENDING'));
                // Content edits of public listings return to PENDING for Admin re-review.
                $newStatus = $currentStatus === 'AVAILABLE' ? 'PENDING' : $currentStatus;
                // Never accept AVAILABLE/SOLD from seller POST; keep or soften only as above.
                ...
                flash_set(
                    'success',
                    $newStatus === 'PENDING' && $currentStatus === 'AVAILABLE'
                        ? 'Property updated and returned to PENDING for Admin review.'
...
$old = [
    ...
    'perch' => $property['perch'] !== null && isset($property['perch']) ? (string) $property['perch'] : '',
    'bedrooms' => isset($property['bedrooms']) && $property['bedrooms'] !== null ? (string) $property['bedrooms'] : '',
    'bathrooms' => isset($property['bathrooms']) && $property['bathrooms'] !== null ? (string) $property['bathrooms'] : '',
    'kitchen_area_sqft' => isset($property['kitchen_area_sqft']) && $property['kitchen_area_sqft'] !== null ? (string) $property['kitchen_area_sqft'] : '',
    ...
    'floors' => isset($property['floors']) && $property['floors'] !== null ? (string) $property['floors'] : '',
    'year_built' => isset($property['year_built']) && $property['year_built'] !== null ? (string) $property['year_built'] : '',
    'asking_price_lkr' => isset($property['asking_price_lkr']) ? (string) $property['asking_price_lkr'] : '',
];
...
$flash = flash_get();
$page_title = 'Edit Property | RealEstateAI';
$page_description = 'Edit your RealEstateAI property listing.';
?>
    ...
    <div class="container">
        ...
        <?php if ($notFound || $property === null): ?>
            <div class="admin-hero">
                ...
                <p class="admin-welcome">You can only edit listings that you own.</p>
            </div>
        ...
        <?php else: ?>
            <div class="admin-hero">
                ...
                <h1 class="admin-title">Edit Property</h1>
            ...
            <div class="admin-form-card admin-form-card-wide mb-4">
                <form method="post" action="" novalidate>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <a class="btn btn-outline-secondary" href="<?php echo e(url('seller/property_view.php?id=' . $propertyId)); ?>">Cancel</a>
                    </div>
                </form>
            </div>
                    ...
                    <div class="property-gallery mb-3">
                        <?php foreach ($images as $image): ?>
                            ...
                            <figure class="property-gallery-item<?php echo $isPrimary ? ' is-primary' : ''; ?>">
                                <?php if ($imgUrl !== null): ?>
                                    <img src="<?php echo e($imgUrl); ?>" alt="<?php echo e($isPrimary ? 'Primary property image' : 'Property gallery image'); ?>" loading="lazy">
                                <?php endif; ?>
                                <?php if ($isPrimary): ?>
